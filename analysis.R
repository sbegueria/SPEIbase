...
spei.nc <- function(sca, inPre, inEtp=NA, inMask=NA, block=18, tlapse=NA) {
  ...
  # Create the variable and output file
  if (!is.na(tlapse[1])) {
  ...
  spi.nc.var <- ncvar_def(
    ...
    prec='float',
    compression=9)
  spi.nc <- nc_create(paste('./spei', sca, '.nc', sep=''), spi.nc.var)
  ncatt_put(spi.nc, nam, attname='description',
            attval=paste('Using ub-pwm method for parameter estimation',sep=''))
  ncatt_put(spi.nc, nam, attname='version', attval='2.1')
  ncatt_put(spi.nc, nam, attname='creator',
            attval='Santiago Begueria <santiago.begueria@csic.es>')
  ncatt_put(spi.nc, 'lon', attname='limits',
            attval=paste(spi.nc$dim$lon$vals[1],
                         spi.nc$dim$lon$vals[spi.nc$dim$lon$len],sep=', '))
  ncatt_put(spi.nc, 'lat', attname='limits',
            attval=paste(spi.nc$dim$lat$vals[1],
                         spi.nc$dim$lat$vals[spi.nc$dim$lat$len],sep=', '))
  ncatt_put(spi.nc, 'time', attname='limits',
            attval=paste(spi.nc$dim$time$vals[1],
                         spi.nc$dim$time$vals[spi.nc$dim$time$len],sep=', '))
  ncatt_put(spi.nc, nam, attname='description',
            attval='Coordinates are referred to cell centres')
...
inPre <- './inputData/cru_ts_3_10_01.1901.2009.pre.dat.nc'
inPen <- './inputData/cru_ts_3_10.1901.2009.pet.dat.nc'
#inMask <- './mask.nc'

library(snowfall)
sfInit(parallel=TRUE, cpus=22)
for (i in 1:48) {
  spei.nc(i, inPre, inPen, block=12)
}
sfStop()
